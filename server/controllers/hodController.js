...
async function listForms(req, res, next) {
  try {
    ...
    for (const u of users) {
      const forms = (u.forms || []).filter(
        (f) => Number(f.formYear) === Number(year) && f.status === 'SUBMITTED'
      )
      if (forms && forms.length) {
        ...
        try {
          const articleIds = (form.research || [])
            .map((r) => (r.article ? String(r.article) : null))
            .filter(Boolean)
          ...
          const entries = (form.research || []).map((r, index) => {
            ...
            return {
              ...
              dateOfPublication:
                (art && art.dateOfPublication) || r.dateOfPublication || null,
              authorRole:
                (art && art.metadata && art.metadata.authorRole) ||
                (r && r.metadata && r.metadata.authorRole) ||
                r.authorRole ||
                '',
          ...
          form.entries = (form.research || []).map((r, index) => ({
            index: index + 1,
            title: r.title || '',
            journalName: r.journalName || '',
            issnIsbn: r.issnIsbn || '',
            indexedIn: r.indexedIn || '',
            dateOfPublication: r.dateOfPublication || null,
            authorRole:
              (r && r.metadata && r.metadata.authorRole) ||
              r.authorRole ||
              '',
...
async function approveSection(req, res, next) {
  try {
    ...
    if (!facultyId || !formYear || !sectionName || !status) {
    ...
    const form = (faculty.forms || []).find(
      (f) => f.formYear === Number(formYear)
    )
    if (!form) return res.status(404).json({ message: 'Form not found' })

    // ----- store / update this section's approval record -----
    ...
    // Currently this endpoint is used by HOD; any REJECTED here means
    // the whole form is rejected.
    if (status === 'REJECTED') {
      form.status = 'REJECTED'

      // 1) Delete uploaded proof files on disk
      try {
      ...
      // 2) 🔴 CLEAR ALL RESEARCH ENTRIES for this form
      //    (what you requested)
      form.research = []
      form.totalScore = 0 // optional but makes sense for a rejected form
